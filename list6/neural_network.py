...
class NeuralNetwork:
    ...
    def _feed_forward(self, with_training_data: bool = True) -> None:
        ...
        for layer, next_layer in zip(self.hidden_layers[0:-1], self.hidden_layers[1:]):
            # print(layer.info.neurons_count, layer.info.activation_function_name, '->',
            #       next_layer.info.neurons_count)
            next_layer.values = layer.utils.function(
                np.dot(layer.values, layer.weights.T)
            )
        last_hidden_layer = self.hidden_layers[-1]
        # print(last_hidden_layer.info.neurons_count, '->', self.output.shape)
    ...
    def learn(self, iterations: int, draw=None) -> None:
        """
        ...
        for i in range(iterations):
            ...
            if draw is not None and i % draw == 0:
                plt.scatter(self.training_data_sets, self.output, 10)
